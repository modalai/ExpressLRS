...
void CRSFEndpoint::sendDeviceInformationPacket()
{
    ...
    device->hardwareVer = htobe32(HARDWARE_REV);
#else
...
#if defined(M0139)
    static bool firstDeviceInformation = true;
    device->parameterVersion = firstDeviceInformation ? 0x80 : 0;
    firstDeviceInformation = false;
#else
    ...
    {
        responseConnector->forwardMessage((crsf_header_t *)deviceInformation);
        responseConnector = nullptr;
    }
    else
    {
